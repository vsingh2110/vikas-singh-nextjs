      ...
      <div className="max-w-[1300px] mx-auto px-4 sm:px-6 md:px-10 lg:px-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service, index) => (
            <div
              key={index}
              className="bg-gray-50 rounded-md p-8 md:p-10 lg:p-12 text-center cursor-pointer transition-all duration-300 hover:bg-[#dc143c] hover:scale-105 group border border-gray-200"
            >
              ...
              <h3 className="text-xl sm:text-2xl font-medium mb-3 sm:mb-4 text-gray-800 group-hover:text-white transition-colors duration-300">{service.title}</h3>
              <p className="text-sm sm:text-base text-gray-700 group-hover:text-white transition-colors duration-300">{service.description}</p>
            </div>
          ))}
        </div>
      </div>
